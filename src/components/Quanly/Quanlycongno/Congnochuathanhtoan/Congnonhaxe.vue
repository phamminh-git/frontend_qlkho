    ...
    <table class="table table-striped table-bordered" v-if="!loading">
      ...
      <tbody>
        <tr v-for="(getGarage, index) in getGarages" :key="(getGarage.id,index)">
          ...
          <td>
            {{getGarage['totalDebt']| soTien | dinhdangtien | vietnamDong}}
          </td>
          <td>
            <router-link :to="{ path: '/dashboardadmin/congnochuathanhtoan/congnonhaxe/congnocacxe/' + getGarage['id']}" class="router-link">
            Danh sách các xe
            </router-link>
          </td>
        </tr>
