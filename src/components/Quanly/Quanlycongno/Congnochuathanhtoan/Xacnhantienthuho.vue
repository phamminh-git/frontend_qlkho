<template>
  <div class="pt-2">
    ...
    <div class="alert alert-success" v-if="confirmSuccess">Nhận tiền thu hộ của mặt hàng thành công</div>
    ...
    <table class="table table-striped table-bordered" v-if="!loading">
      ...
      <tbody>
        <tr v-for="(good,index) in goods" :key="index">
          ...
          <td>
            <b-button variant="success" class="btn-sm" @click="showConfirmModal(good.id)">Xác nhận</b-button>
            <router-link
    ...
    <b-modal title="Thanh toán" ref="confirm-modal" hide-footer>
      <p class="mb-4">Xác nhận tiền thu hộ</p>
      <div class="text-end">
        <b-button variant="danger" class="mx-2 btn-sm" @click="hideConfirmModal">Huỷ bỏ</b-button>
        <b-button variant="success"  class="btn-sm" @click="confirm()">Xác nhận</b-button>
      </div>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    getlistGoods(page) {
      this.loading = true;
      BaseRequest.get('/get-goods-not-pay-ware-house/' + this.id + '?per_page=' + this.perPage, {page})
      .then( response => {
        this.loading = false;
    ...
    confirm: function(){
      BaseRequest.put('/confirm-collected-money-from-car/' + this.good_id)
      .then( () => {
        this.$refs["confirm-modal"].hide();
        setTimeout(() => {
